...
export function UploadPrompt() {
  ...
  const dropzone = useDropzone({
    ...
    onDropRejected: () => {
      pushToast({ description: t.uploadPrompt.toast.rejected.desc, title: t.uploadPrompt.toast.rejected.title });
    },
  });
    ...
    <div className="flex min-h-[calc(100vh-80px)] flex-col items-center justify-center px-4 py-16">
      <div className="w-full max-w-lg space-y-10">
        <div className="space-y-3 text-center">
          ...
          <h2 className="text-3xl font-semibold tracking-tight">{t.uploadPrompt.title}</h2>
          <p className="text-sm leading-7 text-[var(--muted-foreground)]">{t.uploadPrompt.desc}</p>
        </div>
        ...
        >
          ...
          <div className="flex flex-col items-center gap-4">
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[var(--accent-soft)] text-[var(--accent)]">
              <svg className="h-7 w-7" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.75} viewBox="0 0 24 24">
                <path d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M16 12l-4-4-4 4M12 8v8" />
              </svg>
            </div>
            <div>
              <p className="font-semibold">
                {dropzone.isDragActive ? t.uploadPrompt.release : t.uploadPrompt.drag}
              </p>
              <p className="mt-1 text-sm text-[var(--muted-foreground)]">{t.uploadPrompt.accepts}</p>
            </div>
            <Button variant="secondary">{t.uploadPrompt.chooseFile}</Button>
          </div>
        </div>
            ...
            {t.sampleCards.items.map((sample) => (
              <div className="group relative" key={sample.fileName}>
                ...
                >
                  ...
                  <p className="mt-1.5 text-sm font-medium text-[var(--foreground)]">{sample.title}</p>
                </button>
                <a
