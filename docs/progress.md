...
**當前階段**：`P2.6`（待開始）— P2.5 重新整理、新建、刪除完成
...
## P2.5 — 重新整理、新建、刪除
**完成日期**：2026-05-05
**Commit**：待提交 `feat(p2.5): 支援資料夾重新整理、新建、刪除`
**驗收**：✅ lint / ✅ test / ✅ build / ✅ 手動驗收

### 本子階段完成項目
- 側邊欄 Refresh 會重新 list 目前授權資料夾並更新 tree
- 新增 `New` dialog，可輸入檔名並套用 P1.5.3 的空白 / 閱讀 / 考試 / 簡報模板
- 新建檔案透過 `FSAccessAdapter.write()` 建立並寫入內容，建立後自動讀入與導頁
- `FSAccessAdapter.write()` 改為 `getFileHandle(path, { create: true })`，支援新檔案寫入
- 每個檔案列提供 `Del` 操作，需先切到 `Delete?` 確認狀態才會真正刪除
- 刪除目前開啟檔案時會清 document / exam / slides 狀態並回首頁
- 補上 adapter 新檔案寫入測試

### 遇到的問題
- 檔案列不能放 nested button，因此改成 row container + open button + delete button，避免互動語意錯誤
- 新建目前先放在資料夾 root；巢狀資料夾內新建可等實際使用後再補細節

### 心得 / 決策
- 刪除先採二段確認，沒有做 undo；這符合 P2.5 的安全底線，也避免引入暫存回收機制

### 下一步
- 進入 P2.6 階段 2 驗收與心得記錄

## P2.4 — 寫回檔案
**完成日期**：2026-05-05
**Commit**：`4608b6a feat(p2.4): 實作編輯後寫回原檔`
**驗收**：✅ lint / ✅ test / ✅ build / ✅ 手動驗收
